...
def build_move_details(player: int, move: Move, state_before: GameState, state_after: GameState) -> dict:
    ...
    if isinstance(move, ShowMove):
        ...
        return {
            # Slice from the pre-move hand so the log shows the cards that were played
            "cards": tuple(state_before.hands[player][start:end]),
            "score_delta": score_delta,
        }

    if isinstance(move, ScoutMove):
        scout_card = _scouted_card(state_before, move.candidate.table_index)
        return {
    ...
    if isinstance(move, ScoutAndShowMove):
        # Combined branch
        scout_card = _scouted_card(state_before, move.candidate.scout.table_index)
...
def _scouted_card(state: GameState, table_index: int) -> Optional[Card]:
